    ...
    <div class="wrap">
      <div class="stat-row">
        <div class="stat" v-reveal>
          Cartes<b>{{ collection.total_cards }}</b>
        </div>
        <div class="stat" v-reveal="1">
          Uniques<b>{{ collection.unique_cards }}</b>
        </div>
          ...
          <tbody>
            <tr v-for="item in collection.items" :key="item.card.id">
              ...
              <td style="white-space: nowrap">
                <button class="btn btn-ghost btn-sm" @click="setQty(item, 1)" aria-label="Ajouter un exemplaire">
                  +
                </button>
                <button class="btn btn-ghost btn-sm" @click="setQty(item, -1)" aria-label="Retirer un exemplaire">
                  −
                </button>
              </td>
            </tr>
