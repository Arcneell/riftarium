      ...
      <div class="builder">
            ...
            <div class="deck-grid" v-if="grouped[zone.key].length">
              ...
              >
                <RouterLink :to="`/cartes/${entry.card.id}`">
                  <img
                    :src="cardThumb(entry.card.image_url, 240)"
                ...
                <div class="dcard-actions">
                  <button
                    @click="setQty(entry.card.id, -1)"
                    :aria-label="`Retirer un exemplaire de ${entry.card.name}`"
                  >
                    −
                  </button>
                  <button @click="setQty(entry.card.id, 1)" :aria-label="`Ajouter un exemplaire de ${entry.card.name}`">
                    +
                  </button>
                </div>
              </div>
            </div>
            <p v-else class="muted" style="font-size: 0.85rem">Rien pour l'instant — cherchez une carte à droite.</p>
          </template>
        ...
        <div class="builder-side">
          <div class="panel">
            ...
            <p v-else-if="searchQuery.length >= 2" class="muted" style="font-size: 0.85rem">Aucun résultat.</p>
            <p v-else class="muted" style="font-size: 0.85rem">Un clic sur une carte l'ajoute au deck.</p>
          </div>
          ...
          <div class="panel">
            <h3 style="margin-bottom: 14px">
              ...
              <span class="muted" style="font-size: 0.75rem">{{
                deck.format === "tournament" ? "règles de tournoi" : "mode libre, à titre indicatif"
              }}</span>
            </h3>
            <ul class="validator">
