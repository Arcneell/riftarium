...
watch(
  () => [state.set_id, state.type, state.domain],
  () => {
    state.page = 1
    load()
  }
    ...
    <div class="wrap">
      ...
      <div class="toolbar">
        <div class="filters" role="group" aria-label="Filtrer par domaine">
          <button
            v-for="(d, key) in DOMAINS"
            :key="key"
            v-show="key !== 'Colorless'"
            class="filter"
            :style="{ '--chip': d.color }"
            :aria-pressed="state.domain === key"
      ...
      <p class="muted mono" style="font-size: 0.82rem; margin-bottom: 24px">
      ...
      <div class="pager" v-if="result.total > SIZE">
        ...
        <button
          ...
          :disabled="state.page >= Math.ceil(result.total / SIZE)"
          @click="state.page++"
        >
          Suivant →
        </button>
